...
@Component({
  selector: 'app-widget-page', imports: [DatePipe],
  template: `
    <main class="widget">
      ...
      <section class="content">
        @if (type() === 'todo') {
          ...
          @for (todo of store.todos(); track todo.id) { <div class="todo"><span></span><strong>{{ todo.text }}</strong><time>{{ todo.createdAt | date:'MMM d' }}</time></div> } @empty { <p class="muted">Nothing to do.</p> }
        } @else if (type() === 'today') {
  ...
  styles: `
    ...
    .row, .todo { min-height: 54px; display: flex; align-items: center; gap: 11px; border-bottom: 1px solid var(--app-border); font-size: 12px; } .row > time { width: 42px; display: flex; flex-direction: column; gap: 2px; color: var(--app-text); } .row > time span, .row > div span { color: var(--app-muted); font-size: 10px; } .row > div { min-width: 0; display: flex; flex-direction: column; gap: 3px; } .row > div strong { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; } .todo > span { width: 15px; height: 15px; flex: 0 0 15px; border: 1px solid var(--app-muted); border-radius: 50%; } .todo > strong { flex: 1; } .todo time { color: var(--app-muted); font-size: 10px; }
...
export class WidgetPage {
  readonly store = inject(LocalStore); readonly clock = inject(ClockService); private readonly route = inject(ActivatedRoute);
