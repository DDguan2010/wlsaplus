...
@Component({
  ...
  template: `
    <div class="page tool-page">
      ...
      @if (platform.info.kind === 'android') { <p class="platform-note">Android opens the WLSAPlus relay in an installed Clash-compatible client.</p> }
  ...
  styles: `
    ...
    .mode-row { min-height: 52px; display: flex; justify-content: flex-end; align-items: center; gap: 6px; color: var(--app-muted); font-size: 13px; font-weight: 700; } .mode-row .material-symbols-rounded { font-size: 18px; }
...
export class VpnPage {
  ...
  setSource(event: Event): void { this.vpn.setSource((event.target as HTMLSelectElement).value); }
